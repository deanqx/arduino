...
#define __GLCD_Pointer(X, Y)                                                   \
  (X + ((Y / __GLCD_Screen_Line_Height) * __GLCD_Screen_Width))
//---------------------------//

//----- Prototypes ----------------------------//
static void GLCD_Send(const uint8_t Control, uint8_t *Data,
                      const uint8_t Length);
static void GLCD_BufferWrite(const uint8_t X, const uint8_t Y,
                             const uint8_t Data);
static uint8_t GLCD_BufferRead(const uint8_t X, const uint8_t Y);
static void GLCD_DrawHLine(uint8_t X1, uint8_t X2, const uint8_t Y,
                           enum Color_t Color);
static void GLCD_DrawVLine(uint8_t Y1, uint8_t Y2, const uint8_t X,
                           enum Color_t Color);
...
// Reset if needed
  ...
  GLCD_SendCommand(__GLCD_Command_Display_Start_Line_Set |
  ...
  GLCD_SendCommand(__GLCD_Command_Segment_Remap_Set |
                   0x01); // 0xA0 - Left towards Right

  GLCD_SendCommand(__GLCD_Command_Com_Output_Scan_Dec); // 0xC8 - Up towards
                                                        // Down
  ...
  __GLCD.X = __GLCD.Y = __GLCD.Font.Width = __GLCD.Font.Height =
      __GLCD.Font.Lines = 0;
}
...
enum GLCD_Status_t GLCD_GetStatus(void) { return (__GLCD.Status); }
#endif
...
void GLCD_Clear(void) { GLCD_FillScreen(GLCD_White); }
...
uint8_t GLCD_GetX(void) { return __GLCD.X; }

uint8_t GLCD_GetY(void) { return __GLCD.Y; }

uint8_t GLCD_GetLine(void) { return (__GLCD_GetLine(__GLCD.Y)); }
...
void GLCD_SetPixels(const uint8_t X1, uint8_t Y1, const uint8_t X2,
                    const uint8_t Y2, enum Color_t Color) {
...
void GLCD_DrawBitmap(const uint8_t *Bitmap, uint8_t Width, const uint8_t Height,
  ...
  lines = bmpRead = bmpReadPrev = x = y = i = j = overflow = data = dataPrev =
      0;
  ...
  lines = (Height + __GLCD_Screen_Line_Height - 1) /
          __GLCD_Screen_Line_Height; // lines = Ceiling(A/B) = (A+B-1)/B
  data =
      __GLCD.Y / __GLCD_Screen_Line_Height + lines; //"data" is used temporarily
  ...
  for (j = 0; j < lines; j++) {
    ...
    bmpRead = j * Width;
    bmpReadPrev =
        bmpRead - Width; // Previous = 4 + (j - 1) * width = Current - width
  ...
  if (__GLCD_GetLine(y2) != __GLCD_GetLine((y2 + Height - 1)) &&
      y < __GLCD_Screen_Height) {
    // Go to the start of the line
    GLCD_GotoXY(x, y);
...
void GLCD_DrawRectangle(const uint8_t X1, const uint8_t Y1, const uint8_t X2,
                        const uint8_t Y2, enum Color_t Color) {
  if ((X1 < __GLCD_Screen_Width) && (X2 < __GLCD_Screen_Width) &&
      (Y1 < __GLCD_Screen_Height) && (Y2 < __GLCD_Screen_Height)) {
    GLCD_DrawHLine(X1, X2, Y1, Color);
...
void GLCD_DrawRoundRectangle(const uint8_t X1, const uint8_t Y1,
                             const uint8_t X2, const uint8_t Y2,
                             const uint8_t Radius, enum Color_t Color) {
  if ((X1 < __GLCD_Screen_Width) && (X2 < __GLCD_Screen_Width) &&
      (Y1 < __GLCD_Screen_Height) && (Y2 < __GLCD_Screen_Height)) {
...
void GLCD_DrawTriangle(const uint8_t X1, const uint8_t Y1, const uint8_t X2,
                       const uint8_t Y2, const uint8_t X3, const uint8_t Y3,
                       enum Color_t Color) {
  if (((X1 < __GLCD_Screen_Width) && (X2 < __GLCD_Screen_Width) &&
       (X3 < __GLCD_Screen_Width) && (Y1 < __GLCD_Screen_Height) &&
       (Y2 < __GLCD_Screen_Height) && (Y3 < __GLCD_Screen_Height))) {
...
void GLCD_DrawCircle(const uint8_t CenterX, const uint8_t CenterY,
                     const uint8_t Radius, enum Color_t Color) {
...
void GLCD_FillRectangle(const uint8_t X1, const uint8_t Y1, const uint8_t X2,
                        const uint8_t Y2, enum Color_t Color) {
  GLCD_SetPixels(X1, Y1, X2, Y2, Color);
}

void GLCD_FillRoundRectangle(const uint8_t X1, const uint8_t Y1,
                             const uint8_t X2, const uint8_t Y2,
                             const uint8_t Radius, enum Color_t Color) {
  if ((X1 < __GLCD_Screen_Width) && (X2 < __GLCD_Screen_Width) &&
      (Y1 < __GLCD_Screen_Height) && (Y2 < __GLCD_Screen_Height)) {
    ...
    while (x <= y) {
      // Left side
      GLCD_DrawLine(X1 + Radius - x,
                    Y1 + Radius - y, // Upper left corner upper half
                    X1 + Radius - x,
                    Y1 + height - Radius + y, // Lower left corner lower half
                    Color);
      GLCD_DrawLine(X1 + Radius - y,
                    Y1 + Radius - x, // Upper left corner lower half
                    X1 + Radius - y,
                    Y1 + height - Radius + x, // Lower left corner upper half
                    Color);

      // Right side
      GLCD_DrawLine(X1 + width - Radius + x,
                    Y1 + Radius - y, // Upper right corner upper half
                    X1 + width - Radius + x,
                    Y1 + height - Radius + y, // Lower right corner lower half
                    Color);
      GLCD_DrawLine(X1 + width - Radius + y,
                    Y1 + Radius - x, // Upper right corner lower half
                    X1 + width - Radius + y,
                    Y1 + height - Radius + x, // Lower right corner upper half
                    Color);
...
void GLCD_FillTriangle(uint8_t X1, uint8_t Y1, uint8_t X2, uint8_t Y2,
                       uint8_t X3, uint8_t Y3, enum Color_t Color) {
  if (((X1 < __GLCD_Screen_Width) && (X2 < __GLCD_Screen_Width) &&
       (X3 < __GLCD_Screen_Width) && (Y1 < __GLCD_Screen_Height) &&
       (Y2 < __GLCD_Screen_Height) && (Y3 < __GLCD_Screen_Height))) {
    uint8_t sl, sx1, sx2;
...
void GLCD_FillCircle(const uint8_t Center, const uint8_t CenterY,
                     const uint8_t Radius, enum Color_t Color) {
  if (((Center + Radius) < __GLCD_Screen_Width) &&
    ...
    while (x < y) {
      ...
      // perimeter points on the upper and lower quadrants of the 2 halves of
      // the circle.
...
void GLCD_SetFont(const uint8_t *Name, const uint8_t Width,
                  const uint8_t Height, enum PrintMode_t Mode) {
  if ((Width < __GLCD_Screen_Width) && (Height < __GLCD_Screen_Height) &&
      ((Mode == GLCD_Overwrite) || (Mode == GLCD_Merge))) {
...
uint8_t GLCD_GetWidthChar(const char Character) {
  ...
  return (pgm_read_byte(&(
              __GLCD.Font.Name[(Character - 32) *
                               (__GLCD.Font.Width * __GLCD.Font.Lines + 1)])) +
          1);
}
...
void GLCD_PrintChar(char Character) {
  ...
  fontStart = fontRead = fontReadPrev = x = y = y2 = i = j = width = overflow =
      data = dataPrev = 0;
  ...
  fontStart =
      Character * (__GLCD.Font.Width * __GLCD.Font.Lines +
                   1); //+1 due to first byte of each array line being the width
  ...
  // If (LINE_STARTING != LINE_ENDING)
  if (__GLCD_GetLine(y2) != __GLCD_GetLine((y2 + __GLCD.Font.Height - 1)) &&
      y < __GLCD_Screen_Height) {
    // Go to the start of the line
    GLCD_GotoXY(x, y);
...
#if defined(GLCD_Error_Checking)
    status = __I2C_Status();
    if ((status != MT_START_TRANSMITTED) &&
        (status != MT_REP_START_TRANSMITTED)) {
...
#if defined(GLCD_Error_Checking)
    status = __I2C_Status();
    if ((status != MT_SLA_W_TRANSMITTED_ACK) &&
        (status != MT_SLA_W_TRANSMITTED_NACK)) {
      __GLCD.Status = GLCD_Error;
      break;
    }
#endif
...
#if defined(GLCD_Error_Checking)
    status = __I2C_Status();
    if ((status != MT_DATA_TRANSMITTED_ACK) &&
        (status != MT_DATA_TRANSMITTED_NACK)) {
      __GLCD.Status = GLCD_Error;
      break;
    }
#endif
...
static inline void GLCD_DrawHLine(uint8_t X1, uint8_t X2, const uint8_t Y,
                                  enum Color_t Color) {
  if (X1 > X2)
    __GLCD_Swap(X1, X2);
...
static inline void GLCD_DrawVLine(uint8_t Y1, uint8_t Y2, const uint8_t X,
                                  enum Color_t Color) {
  if (Y1 > Y2)
    __GLCD_Swap(Y1, Y2);
