 ...
 *
 * lcd library for ssd1306/ssd1309/sh1106 oled-display is free software: you can
 * redistribute it and/or modify it under the terms of the GNU General Public
 * License as published by the Free Software Foundation, either version 3 of the
 * License, or any later version.
 *
 * lcd library for ssd1306/ssd1309/sh1106 oled-display is distributed in the
 ...
 *
 * lcd library for ssd1306/ssd1309/sh1106 oled-display ist Freie Software: Sie
 * können es unter den Bedingungen der GNU General Public License, wie von der
 * Free Software Foundation, Version 3 der Lizenz oder jeder späteren
 * veröffentlichten Version, weiterverbreiten und/oder modifizieren.
 *
 * lcd library for ssd1306/ssd1309/sh1106 oled-display wird in der Hoffnung,
 ...
 * EINEN BESTIMMTEN ZWECK. Siehe die GNU General Public License für weitere
 * Details.
 *
...
#error                                                                         \
    "This library requires AVR-GCC 3.3 or later, update to newer AVR-GCC compiler !"
#endif
...
#define LCD_I2C_ADR                                                            \
  (0x78 >> 1) // 7 bit slave-adress without r/w-bit
...
void lcd_puts(const char *s); // print string, \n-terminated, from ram on screen
                              // (TEXTMODE) or buffer (GRAPHICMODE)
...
void lcd_gotoxy(uint8_t x,
                uint8_t y); // set curser at pos x, y. x means character,
// y means line (page, refer lcd manual)
void lcd_goto_xpix_y(uint8_t x,
                     uint8_t y); // set curser at pos x, y. x means pixel,
// y means line (page, refer lcd manual)
...
uint8_t lcd_drawLine(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2,
                     uint8_t color);
uint8_t lcd_drawRect(uint8_t px1, uint8_t py1, uint8_t px2, uint8_t py2,
                     uint8_t color);
uint8_t lcd_fillRect(uint8_t px1, uint8_t py1, uint8_t px2, uint8_t py2,
                     uint8_t color);
uint8_t lcd_drawCircle(uint8_t center_x, uint8_t center_y, uint8_t radius,
                       uint8_t color);
uint8_t lcd_fillCircle(uint8_t center_x, uint8_t center_y, uint8_t radius,
                       uint8_t color);
uint8_t lcd_drawBitmap(uint8_t x, uint8_t y, const uint8_t picture[],
...
uint8_t
lcd_check_buffer(uint8_t x,
                 uint8_t y); // read a pixel value from the display buffer
void lcd_display_block(uint8_t x, uint8_t line,
                       uint8_t width); // display (part of) a display line
#endif
